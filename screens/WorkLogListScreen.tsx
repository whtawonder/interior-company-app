...
type WorkLogWithProject = { id: string; project_id: string; work_date: string; work_content: string; cost: number; work_cate1: string; worker_name: string; notes: string | null; project: { project_name: string; client_name: string } }
...
export default function WorkLogListScreen({ route, navigation }: any) {
  ...
  useEffect(() => { if (!loading) { setSelectedCategory('all'); fetchWorkLogs() } }, [selectedProject])
  useEffect(() => { if (!loading) { fetchWorkLogs() } }, [selectedCategory])

  const loadData = async () => { setLoading(true); await Promise.all([fetchProjects(), fetchWorkLogs()]); setLoading(false) }
  const fetchProjects = async () => { const { data, error } = await supabase.from('projects').select('id, project_name, client_name, status').in('status', ['estimate', 'in_progress']).order('created_at', { ascending: false }); if (!error) { setProjects(data || []) } }

  const fetchWorkLogs = async () => {
    let query = supabase.from('work_logs').select('id, project_id, work_date, work_content, cost, work_cate1, worker_name, notes, projects:project_id (project_name, client_name)').order('work_date', { ascending: false })
    if (selectedProject !== 'all') { query = query.eq('project_id', selectedProject) }
    if (selectedCategory !== 'all') { query = query.eq('work_cate1', selectedCategory) }
    const { data, error } = await query
    if (error) { alert('작업일지를 불러올 수 없습니다') } else {
      const formattedData = (data || []).map((log: any) => ({ ...log, project: log.projects }))
      ...
      if (selectedProject !== 'all') { catQuery = catQuery.eq('project_id', selectedProject) }
      const catData = await catQuery
      if (catData.data) {
        const uniqueCats = Array.from(new Set(catData.data.map(log => log.work_cate1))).map(catName => ({ id: catName, category_name: catName }))
        setCategories(uniqueCats)
      }
    }
  }

  const handleEdit = (log: WorkLogWithProject) => { navigation.navigate('작업일지 입력', { editMode: true, logData: { id: log.id, project_id: log.project_id, work_date: log.work_date, work_content: log.work_content, cost: log.cost, work_cate1: log.work_cate1, worker_name: log.worker_name, notes: log.notes || '기본' } }) }
  const handleDelete = (id: string) => { Alert.alert('삭제 확인', '이 작업일지를 삭제하시겠습니까?', [{ text: '취소', style: 'cancel' }, { text: '삭제', style: 'destructive', onPress: async () => { const { error } = await supabase.from('work_logs').delete().eq('id', id); if (error) { alert('삭제 실패') } else { alert('삭제되었습니다'); fetchWorkLogs() } } }]) }
  const formatCurrency = (amount: number) => '₩' + amount.toLocaleString('ko-KR')
  const handleLayout = (id: string, y: number) => { setItemLayouts(prev => ({ ...prev, [id]: y })) }
  ...
  return (
    <SafeAreaView style={s.sa} edges={['top']}>
      <View style={s.co}>
        <View style={s.h}>
          <Text style={s.ti}>작업일지 목록</Text>
          <View style={s.sr}>
            <Text style={s.tt}>총 {workLogs.length}건 · {formatCurrency(totalCost)}</Text>
            {selectedCategory !== 'all' && totalCost > 0 && (<Text style={s.ct}>{selectedCategory} 합계: {formatCurrency(totalCost)}</Text>)}
          </View>
        </View>
        <View style={s.fc}>
          <Text style={s.fl}>프로젝트:</Text>
          <View style={s.fp}><RNPickerSelect value={selectedProject} onValueChange={(v) => setSelectedProject(v)} items={[{ label: '전체', value: 'all' }, ...(projects || []).map(p => ({ label: p.project_name, value: p.id }))]} style={fps} useNativeAndroidPickerStyle={false} /></View>
        </View>
        <View style={s.fc}>
          <Text style={s.fl}>공정:</Text>
          <View style={s.fp}><RNPickerSelect value={selectedCategory} onValueChange={(v) => setSelectedCategory(v)} items={[{ label: '전체', value: 'all' }, ...(categories || []).map(c => ({ label: c.category_name, value: c.category_name }))]} style={fps} useNativeAndroidPickerStyle={false} /></View>
        </View>
